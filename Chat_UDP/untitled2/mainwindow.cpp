    ...
    , ui(new Ui::MainWindow)
{
    ...
    udpSocket->bind(QHostAddress::LocalHost, 1111);//привязка к адрессу и порту
...
void MainWindow::ReadingData()//вызывается при получении данных
{
    ...
    while (udpSocket->hasPendingDatagrams())//выполнение до конца датаграмм
    {
        ...
        if (i == 0)//условие 1 пакета
            ui->textEdit->append(prefix +"<font color=#71aaeb>Ренат: </font>" + QString(datagram));//дроп текста
        else
...
void MainWindow::on_SendingData_clicked()
{
    ...
    if(!text.isEmpty())
    {
        ...
        for (int i = 0; i < numPackets; ++i) {
            ...
            udpSocket->writeDatagram(packet, QHostAddress::LocalHost, 9999);//отправка даты
        }
        QString information = CountingDate();
        ui->textEdit->append(prefix + "<font color=#71aaeb>Дмитрий: </font>" + QString(text));//вывод отправленного текста в свой чат
    ...
    QString filePath = QFileDialog::getOpenFileName(this, "Выберите файл");//файловый путь
        ...
        if (!filePath.isEmpty()) {//проверка пустоты
            ...
            if (file.open(QIODevice::ReadOnly)) {//открытие файла для чтения
                ...
                udpSocket->writeDatagram(fileData, QHostAddress::LocalHost, 9999);//отправка
